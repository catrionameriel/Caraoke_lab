...
class TestRoom < MiniTest::Test
  ...
  def test_has_guest_checked_out__guest_does_not_exist
    @room1.check_in_guest(@room1, @guest2)
    assert_equal("Sorry, that customer is not checked in", @room1.check_out_guest(@room1, @guest3))
  end
  ...
  def test_enough_room__yes
    assert_equal(true, @room1.enough_room?(@room1))
  end

  def test_enough_room__no
    @room1.check_in_guest(@room1, @guest3)
    @room1.check_in_guest(@room1, @guest2)
    assert_equal(false, @room1.enough_room?(@room1))
  end
